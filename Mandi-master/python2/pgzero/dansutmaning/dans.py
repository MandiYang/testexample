...
def skapa_steg():
    ...
    global visa_nedrakning, ropa_dansa, rounds
    antal = 5
    steglista = []
    ropa_dansa = False
    rounds += 1
    if (rounds % 3) == 0:
        danslangd = danslangd + 1
...
def nasta_steg():
    global danslangd, aktuellt_steg, steg_klara
    if aktuellt_steg < danslangd - 1:
        aktuellt_steg = aktuellt_steg + 1
    else:
        steg_klara = True
    return
...
def update():
    global spelet_slut, aktuellt_steg, steg_klara
    if not spelet_slut:
        if steg_klara:
            skapa_steg()
            steg_klara = False
